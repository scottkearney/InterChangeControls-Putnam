(function() {
    ...
    function setStyle(elem, repl) {
        var style = elem.getAttribute("style").split(";"), newStyle = [];
        for (var i = 0, l = style.length; i < l; i++) {
            var both = style[i].split(":"), key = both[0], value = both[1];
            if (key in repl) {
    ...
    var cssPattern = /\s*(.*?)\s*{(.*?)}+/g, matchPosition = /\.*?position:.*?sticky.*?;/i, getTop = /\.*?top:(.*?);/i, toObserve = [];
    function parse(css) {
        var matches;
    ...
    window.addEventListener("load", function() {
        ...
        links.forEach(function(link) {
            ...
            var href = link.getAttribute("href"), req = new XMLHttpRequest();
