...
namespace Akumina.InterAction
{
    ...
    public class AkuminaBaseWebPart : WebPart
    {
        ...
        {
            get { return Page.Request.QueryString["preview"] != null; }
        }
        ...
        public static Dictionary<string, object> GetPreview(bool isPreview, SPListItem items)
        {
            ...
            {
                var draftVersion = versionItem.Level.ToString().Equals("draft", StringComparison.InvariantCultureIgnoreCase);
                //bool currentUserisCreator = user.LoginName == versionItem.CreatedBy.LoginName;

                if ((draftVersion || !versionItem.IsCurrentVersion) && (isPreview ? !draftVersion : draftVersion))
                    continue;
                try
                {
        ...
        public static Dictionary<string, object> GetItem(bool rootWeb, string listName, string itemTitle, bool isPreview = false, bool useInternalName = false)
        {
            ...
            try
            {
                ...
                {
                    int itemId;
                    var moderationClause = "";
                    var whereClause = @"<Where>{0}</Where>";
                    //if (list.EnableModeration && !isPreview)
                    //{
                    //    moderationClause = @"<Eq><FieldRef Name='_ModerationStatus' /><Value Type='Number'>0</Value></Eq>";
                    //    whereClause = @"<Where><And>{0}</And></Where>";
                    //}
                    var queryWhere = string.Format(whereClause, int.TryParse(itemTitle, out itemId) ?
                        string.Format(@"<Eq><FieldRef Name='ID' /><Value Type='Number'>{0}</Value></Eq>{1}", itemId, moderationClause)
                        : string.Format(@"<Eq><FieldRef Name='Title' /><Value Type='Text'>{0}</Value></Eq>{1}", itemTitle, moderationClause));
        ...
        public string GetIcon(Icons icons)
        {
            Icons icon = icons;
            switch (icon)
            {
        ...
        {
            if (string.IsNullOrEmpty(HttpContext.Current.Request.UserAgent)) return false;
            return HttpContext.Current.Request.UserAgent.ToUpperInvariant().IndexOf("MS SEARCH", StringComparison.Ordinal) > -1;
        }

        public static string GetValue(string fieldName, Dictionary<string, object> row, DataColumnCollection columns)
        {
            return columns.Contains(fieldName) && row[fieldName] != null ? row[fieldName].ToString() : "";
        }

        public static string GetValue(string fieldName, SPListItem row, DataColumnCollection columns)
        {
            return columns.Contains(fieldName) && row[fieldName] != null ? row[fieldName].ToString() : "";
        }

        public static string GetValue(SPListItem row, string fieldName)
        {
            return row.Fields.ContainsField(fieldName) && row[fieldName] != null ? row[fieldName].ToString() : "";
        }

        public static string GetValue(Dictionary<string, object> row, string fieldName)
        {
            return row.ContainsKey(fieldName) && row[fieldName] != null ? row[fieldName].ToString() : "";
        }
    }
    ...
    public class InstructionResponse
    {
            ...
            T defaultValue = default(T))
        {
            try
            {
                ...
                if (defaultValue.GetType().ToString() == "System.Int32")
                {
                    if (columnValue != null)
                    {
    ...
    {
        private const string LoggingSource = "EventLogs_AK";
