...
function commonload() {
    ...
    //Trigger the search, stop the typing on textbox. Core-JS: jquery-done-typing.js
    $('.ia-searchBox').donetyping(function () {
        var val = $(this).val();
        if (val.length < 3)
            val = "";
    ...
    var oldValue = "";
    //Trigger the search when press on backspace key and delete key
    $('.ia-searchBox').on("keyup", function (event) {
        var val = $(this).val();
        if (val.length > 2)
            oldValue = val;
        if ((val.length == 2 && event.keyCode == 8) || event.keyCode == 46 || (oldValue != "" && event.keyCode == 8 && val.length < 2)) {
            oldValue = "";
            getListItems();
        }
    });
    //Execute the search when cut or paste event triggers
    $('.ia-searchBox').on("cut paste", function (event) {
...
function copyOfsearchTrigger() {
    var modfiedUsers = $("#fileModifiedUser").chosen().val();
